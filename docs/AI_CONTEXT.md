...
- **Canonical intelligence bundle:** one cached `sessionIntelligenceBundle()` builds the full intelligence DAG per session (transaction count + sample-data flag + analytics event count). AI Data Pack, analytics export, Storeroom Mind, and stock *Count this next* read from it. **Dependency validation (`ibv.1`):** declarative DAG manifest + post-build validator checks layer presence, dependency order, structural shape, `generatedAt` alignment, duplicate IDs, belief–reasoning links, and non-finite values. Full result on session cache; summary in AI Data Pack `diagnostics.intelligenceBundleValidation` and analytics `intelligenceBundleValidationSummary`. Exports are never blocked. Headless tests `tests/intelligence-bundle.test.js`, `tests/intelligence-bundle-validation.test.js`.
- **Operational context (Phase X1 + X2):** optional structured `reason` on recount txs (gated chip picker, `OPERATIONAL_CAUSES`) and per-line on build substitutions/omissions (`EXCEPTION_CAUSES` volunteer chips: sub vs omit sets; stored as `{ causeId, label, source: "volunteer" }`). `exceptionReasonSummary` in analytics export and `operationalIntelligence.summary`. Analytics: `exception_reason_selected`, `exception_reason_missing`. Operational Reasoning consumption deferred. `commitContext` balance snapshots. Headless tests `tests/operational-context.test.js`, `tests/exception-reason.test.js`.
- **Storeroom Mind (admin intelligence dashboard):** the first in-app surface for the intelligence layers — read-only and **ops-facing**, reachable only through **Admin tools → "What the app is thinking"** (`view = "intelligence"`), never on the volunteer home flow. Renders existing engines (recount calibration `rc.2`, stock belief-state `bs.1`, Value of Information `voi.1`, operational reasoning Phase 4) in plain language instead of raw JSON. Five sections in **trust-first order**: *Can I trust these numbers yet?* (calibration verdict first), *Count this next* (VoI top pick), *Shelf confidence* (belief-state quantities/bands/stock-out risk), *What the app has noticed* (top reasoning conclusions, sentinels filtered), *Stock at risk* (buildable counts + below-line items). An honesty banner flags synthetic or still-provisional data. A sixth **By the numbers** section adds two hand-rolled charts (zero libraries, offline/CSP-safe) drawn only from the real transaction log — *Stock against its reorder line* (horizontal bars) and *Packs built & delivered by month* (stacked SVG bars); the probabilistic layers are deliberately not charted while provisional. Pure render layer (`computeIntelligenceDashboardModel`/`renderIntelligenceDashboard`); reuses compute functions via the `exportAIDataPack` wiring, mutates no stock, adds no persistence. Headless test `tests/intelligence-dashboard.test.js`. The *volunteer-facing* "one whisper" surface for the VoI top pick stays gated behind real calibration data (product decision, not built).
...
| Storeroom Mind admin intelligence dashboard (ops-facing, read-only) | **Accepted** — see `PRODUCT_DECISIONS.md` |
