...
- **Packing habits trends (Phase 2):** windowed behavioural change analysis with separate `trend` and `evidence` fields plus `trendReason`; analytics `packingHabitTrends` (filtered `topThreeInsights`; raw `trendRows` remain in export). Export-only. See `PRODUCT_DECISIONS.md`.
