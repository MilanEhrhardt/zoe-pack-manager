...
- **Packing habits trends (Phase 2):** windowed behavioural change analysis (recent vs previous production builds per pack); trend, `likelyCause`, and human insights in `packingHabits`; `packingHabitTrends` in analytics summary (with filtered `topThreeInsights` — raw `trendRows` remain in export). Export-only. See `PRODUCT_DECISIONS.md`.
