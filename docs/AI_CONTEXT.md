...
- **Recount calibration (feedback loop, `rc.1`):** read-only loop that scores the item-confidence model against ground truth. `commitRecount` stamps `tx.prediction` (model's believed qty + confidence) before applying the count; derived `recountCalibration` (AI Data Pack) and `recountCalibrationSummary` (analytics) report within-tolerance rate, signed-error bias, Brier score, and per-recount verdicts (`confident_correct`/`overconfident`/`flagged_correctly`/`false_alarm`) with `overconfidenceRate`/`catchRate`/`falseAlarmRate`. Synthetic captures excluded; `readyForCalibration` at ≥8 real recounts. Export-only, no volunteer UI. First measurement of whether the intelligence is correct.
