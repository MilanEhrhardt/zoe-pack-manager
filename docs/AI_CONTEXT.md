...
- **Operational context (Phase X1 + X2):** optional structured `reason` on recount txs (gated chip picker) and per-line on build substitutions/omissions; `commitContext` balance snapshots; legacy recount strings migrate on import. Donation/delivery not in scope. Headless test `tests/operational-context.test.js`.
