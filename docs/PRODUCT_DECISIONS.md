...
| **Status** | Accepted — **superseded by the Phase 4 reasoning engine.** `computeBeliefEngine` now derives beliefs via `deriveBeliefsFromReasoning`; the standalone Phase 3B builders (`computeItemBalanceBeliefs`, `computeShortageBeliefs`, `computeHabitBeliefs`, `computeRecipeAlignmentBeliefs`, `computeReadinessBeliefs`) became dead code and were removed 2026-06-27. Shared `be*` helpers remain in use by the Phase 4 path. |
