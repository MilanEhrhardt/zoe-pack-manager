...
**Phase 2 (shipped):** Compares **non-overlapping recent and previous windows** of production builds per pack type (25/20/10 builds). Exports separate **`trend`** (too_early_to_tell, emerging, established, declining, stable) and **`evidence`** (too_early, early, sufficient) plus **`trendReason`** — never `unstable`. Adds evidence-based `likelyCause`, trend confidence, human `insights`, and extends observations with window counts and deltas. Analytics summary includes `packingHabitTrends`. Still export-only — Janet sees nothing new.
